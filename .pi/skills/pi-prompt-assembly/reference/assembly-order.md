...
Section-by-section breakdown of what `buildSystemPrompt` produces, branch by branch. All cites against `packages/coding-agent/src/core/system-prompt.ts` at pi-mono pin `fc51a40d`.

## Entry point

`buildSystemPrompt(options: BuildSystemPromptOptions)` at `system-prompt.ts:28-174`.

Inputs:
- `customPrompt?: string` — if set, the function takes the **customPrompt branch** (`:53-81`). Otherwise the **default-prompt branch** (`:83-174`) builds pi's stock prompt from scratch.
...
- `toolSnippets?: Record<string, string>` — one-line descriptions keyed by tool name. A tool is only listed in `Available tools:` when a snippet is supplied (`:92-93`).
...
3. **`<project_context>` block** — emitted only if `contextFiles.length > 0` (`:61-68`). The block opens with `\n\n<project_context>\n\nProject-specific instructions and guidelines:\n\n`. Each file becomes `<project_instructions path="<absolute-path>">\n<content>\n</project_instructions>\n\n`. The block closes with `</project_context>\n`. **Pre-0.75.0** this section used a `# Project Context` Markdown heading with `## <abs-path>` per file; PRs #4541 (`7577d3b8`) and #4709 (`aad8cf66`) switched both branches to XML tags so models do not ingest prompt content past the boundary.
4. **Skills section** — `formatSkillsForPrompt(skills)` appended only when both conditions hold (`:71-74`):
   - `customPromptHasRead = !selectedTools || selectedTools.includes("read")` (`:71`). I.e. if `selectedTools` is undefined the gate is open; explicit tool lists must include `"read"`.
   - `skills.length > 0`.
5. **Trailing metadata** (`:77-78`):
   - `\nCurrent date: <date>`
...
## Branch 2 — default prompt (lines 83-174)

Order in the assembled string (built into a single `prompt` variable starting at `:132`):

1. **Hard-coded preamble** (`:132-149`): `"You are an expert coding assistant operating inside pi…"`.
2. **`Available tools:`** list (`:93, :134-135`). Built from `selectedTools` filtered to those with a `toolSnippets[name]` entry; falls back to `(none)` when empty (`:90-93`).
3. **Guidelines** (`:96-130, :139-140`). Auto-derived bullets:
   - File-exploration guidelines: bash-only vs bash+grep/find/ls (`:113-117`).
   - Caller-supplied `promptGuidelines` (`:119-124`), deduped via `guidelinesSet` (`:97-104`).
   - Always-on tail: `"Be concise in your responses"` and `"Show file paths clearly when working with files"` (`:127-128`).
4. **Pi documentation block** (`:142-149`). Hard-coded list of `docs/*.md` paths (extensions, themes, skills, prompt-templates, tui, keybindings, sdk, custom-provider, models, packages) plus the absolute paths from `getReadmePath() / getDocsPath() / getExamplesPath()` (`:84-86`).
5. **APPEND_SYSTEM section** (`:151-153`). Same `\n\n` + `appendSystemPrompt` pattern as the customPrompt branch.
6. **`<project_context>` block** (`:155-163`). Identical XML wrapping to customPrompt branch step 3 (same pre-0.75.0 migration note applies).
7. **Skills section** (`:166-168`). Gate is **only** `hasRead` here — `hasRead = tools.includes("read")` at `:110` — so the implicit-`undefined` shortcut from the customPrompt branch does **not** apply. If the caller passes `selectedTools: []` or any list lacking `"read"`, no skills.
8. **Trailing metadata** (`:170-172`). Same as customPrompt branch.
...
**The body of `SKILL.md` is never injected into the system prompt.** It is loaded later, on demand, when the model issues a `read` tool call against `<location>`. That is why the `read`-tool gate at `system-prompt.ts:71` (customPrompt) and `:166` (default) actually disables skills entirely — without `read`, the model cannot follow up on a skill listing.
