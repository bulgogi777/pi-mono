...
The base interface is `ExtensionContext` at `extensions/types.ts:302-338`. Slash-command handlers receive the extended `ExtensionCommandContext` (`extensions/types.ts:353-387`); session-replacement callbacks receive `ReplacedSessionContext` (`extensions/types.ts:394-404`). This file documents the base interface; the extensions are summarized at the end.
...
| `cwd` | `string` | `:305` | Working directory the agent is running in. Read-only; reflects the value passed to `loadExtensions` (`extensions/loader.ts:579`). |
| `sessionManager` | `ReadonlySessionManager` | `:330` | Read-only view (`extensions/types.ts:189-223`) over the active `SessionManager`. Use `sessionManager.getEntries()`, `getLeafId()`, `getEntry(id)`, `getBranch()`, `getTree()`, `getHeader()`, `getLabel(id)` for inspection. To **mutate** session state, use `pi.setLabel`, `pi.setSessionName`, `pi.appendEntry`, or the command-context methods (`fork`, `newSession`, `navigateTree`). The full API is in **pi-sessions**. |
...
| `getContextUsage()` | `() => ContextUsage \| undefined` | `:328` | Returns `{ tokens, max }`-style estimates (`ContextUsage` at `extensions/types.ts:288-298`). Right after compaction (before next assistant message), `tokens` may be `null` — handle accordingly. |
...
`extensions/types.ts:344-379` extends `ExtensionContext` with five session-mutating methods that are only safe under user-initiated commands:
...
`extensions/types.ts:382-396` extends `ExtensionCommandContext` with two messaging methods bound to the **new** session created by `newSession` / `fork` / `switchSession`:
