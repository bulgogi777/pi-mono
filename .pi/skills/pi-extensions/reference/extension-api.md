...
The `pi` argument passed to every extension factory function. All cites against `packages/coding-agent/src/core/extensions/types.ts` at the current pin (`v0.84.1`, `53fa77cc`). The interface itself is `ExtensionAPI` at `types.ts:1198`. Companion runner-side wiring is in `packages/coding-agent/src/core/extensions/runner.ts` and the loader's `createExtensionAPI` factory at `packages/coding-agent/src/core/extensions/loader.ts:~395`.
...
Overload list at `types.ts:1203-1244` (one overload per event). **30** events total (re-counted 2026-08-13 at `v0.84.1`; was documented as 27). The `ExtensionEvent` union at `types.ts:1034-1059` has **25** members — fewer, because `ToolCallEvent` and `ToolResultEvent` are each themselves a union of per-tool variants.
...
`types.ts:1165-1167`. Takes a `ToolDefinition<TParams, TDetails, TState>` (`types.ts:437-486`).
...
`types.ts:1175`. Adds a `/<name>` slash command. `options` shape: `Omit<RegisteredCommand, "name" | "sourceInfo">` (definition at `types.ts:1086-1097`).

- **handler**: `(args: string, ctx: ExtensionCommandContext) => Promise<void>`. `ctx` is the **extended** `ExtensionCommandContext` (`types.ts:353-387`) which adds `waitForIdle`, `newSession`, `fork`, `navigateTree`, `switchSession`, `reload` — these are unsafe to call from non-command contexts.
...
`types.ts:1177-1184`. Binds a `KeyId` (e.g. `"ctrl+b"`) to a handler.
...
`types.ts:1187-1196`. Adds a custom `--<name>` flag to pi's CLI parser.

- **type**: `"boolean" | "string"`. Read back via `pi.getFlag(name)` (`types.ts:1198`) anywhere after parse.
...
`types.ts:1203`. Lets extensions render their own `CustomMessage` payloads in the TUI.

- **renderer**: `MessageRenderer<T>` (`types.ts:1069-1073`) — `(message, options, theme) => Component | undefined`.
...
`types.ts:1288-1328` (with three full inline examples in the JSDoc). `ProviderConfig` shape at `types.ts:1443-1487`.
...
- See `reference/custom-providers.md` for the full contract; see also `pi.unregisterProvider(name)` at `types.ts:1342`.
...
|---|---|---|
| `sendMessage(message, options?)` | `types.ts:1207-1210` | Inject a `CustomMessage` into the session. Optional `triggerTurn` and `deliverAs: "steer" | "followUp" | "nextTurn"`. |
| `sendUserMessage(content, options?)` | `types.ts:1207-1210` | Inject a user message. Always triggers a turn. |
| `appendEntry<T>(customType, data?)` | `types.ts:1227` | Append a `CustomEntry` for state persistence — **not** sent to LLM. See pi-sessions for `CustomEntry` shape. |
| `setSessionName(name)` / `getSessionName()` | `types.ts:1234-1237` | Session display name (`SessionInfoEntry`). |
| `setLabel(entryId, label?)` | `types.ts:1240` | Bookmark/marker on an entry. `undefined` clears. |
| `exec(command, args, options?)` | `types.ts:1243` | Shell out via the same `exec` helper extensions get on `ctx.exec`. |
| `setModel(model)` / `setThinkingLevel(level)` | `types.ts:1259-1267` | Programmatic model/thinking switching. `setModel` returns `false` if no API key resolves. |
| `events: EventBus` | `types.ts:1345` | Cross-extension pub/sub. See `examples/extensions/event-bus.ts`. |
