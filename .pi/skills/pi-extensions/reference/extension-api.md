...
The `pi` argument passed to every extension factory function. All cites against `packages/coding-agent/src/core/extensions/types.ts` at pi-mono `HEAD`. The interface itself is `ExtensionAPI` at `types.ts:1097-1324`. Companion runner-side wiring is in `packages/coding-agent/src/core/extensions/runner.ts` and the loader's `createExtensionAPI` factory at `packages/coding-agent/src/core/extensions/loader.ts:~395`.
...
Overload list at `types.ts:1089-1140` (one overload per event). 27 events total; `ExtensionEvent` union at `types.ts:959-956`.
...
`types.ts:1146-1148`. Takes a `ToolDefinition<TParams, TDetails, TState>` (`types.ts:431-480`).
...
`types.ts:1156`. Adds a `/<name>` slash command. `options` shape: `Omit<RegisteredCommand, "name" | "sourceInfo">` (definition at `types.ts:1075-1086`).

- **handler**: `(args: string, ctx: ExtensionCommandContext) => Promise<void>`. `ctx` is the **extended** `ExtensionCommandContext` (`types.ts:338-373`) which adds `waitForIdle`, `newSession`, `fork`, `navigateTree`, `switchSession`, `reload` — these are unsafe to call from non-command contexts.
...
`types.ts:1158-1165`. Binds a `KeyId` (e.g. `"ctrl+b"`) to a handler.
...
`types.ts:1168-1177`. Adds a custom `--<name>` flag to pi's CLI parser.

- **type**: `"boolean" | "string"`. Read back via `pi.getFlag(name)` (`types.ts:1179`) anywhere after parse.
...
`types.ts:1184`. Lets extensions render their own `CustomMessage` payloads in the TUI.

- **renderer**: `MessageRenderer<T>` (`types.ts:1063-1067`) — `(message, options, theme) => Component | undefined`.
...
`types.ts:1269-1306` (with three full inline examples in the JSDoc). `ProviderConfig` shape at `types.ts:1332-1371`.
...
- See `reference/custom-providers.md` for the full contract; see also `pi.unregisterProvider(name)` at `types.ts:1320`.
...
|---|---|---|
| `sendMessage(message, options?)` | `types.ts:1188-1191` | Inject a `CustomMessage` into the session. Optional `triggerTurn` and `deliverAs: "steer" | "followUp" | "nextTurn"`. |
| `sendUserMessage(content, options?)` | `types.ts:1188-1191` | Inject a user message. Always triggers a turn. |
| `appendEntry<T>(customType, data?)` | `types.ts:1208` | Append a `CustomEntry` for state persistence — **not** sent to LLM. See pi-sessions for `CustomEntry` shape. |
| `setSessionName(name)` / `getSessionName()` | `types.ts:1215-1218` | Session display name (`SessionInfoEntry`). |
| `setLabel(entryId, label?)` | `types.ts:1221` | Bookmark/marker on an entry. `undefined` clears. |
| `exec(command, args, options?)` | `types.ts:1224` | Shell out via the same `exec` helper extensions get on `ctx.exec`. |
| `setModel(model)` / `setThinkingLevel(level)` | `types.ts:1240-1248` | Programmatic model/thinking switching. `setModel` returns `false` if no API key resolves. |
| `events: EventBus` | `types.ts:1323` | Cross-extension pub/sub. See `examples/extensions/event-bus.ts`. |
