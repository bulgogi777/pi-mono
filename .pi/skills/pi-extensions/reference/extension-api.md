...
> ⚠ **The three overloads added between `v0.84.1` and `v0.85.1` are not yet identified in this table** — the count was re-measured mechanically (`.pi/scripts/recount-enumerations.sh`) but the new events have not been named or documented. Treat the per-event table below as complete **as of `v0.84.1`** and re-read `extensions/types.ts` before asserting an event does not exist. Flagged for `gap-scan` in the 2026-09-06 version-log entry.
