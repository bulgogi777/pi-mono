...
| `session_compact_failed` | **New in 0.85.x.** Compaction did **not** complete — it errored, was cancelled, or was aborted. This is the failure counterpart to `session_compact`, which fires only on success; before 0.85.x there was **no extension-side signal for a failed compaction at all** (the failure was visible only on the RPC wire, via `compaction_end`'s `aborted` / `willRetry` flags). Payload: `reason: "manual" \| "threshold" \| "overflow"`, `aborted: boolean`, `willRetry: boolean`, `errorMessage?: string`, and `fromExtension: boolean` — the last is **true when the failing compaction content came from a `session_before_compact` handler**, i.e. an extension caused this. Correlate with `session_before_compact` on `reason` + `willRetry`. | `SessionCompactFailedEvent` (`extensions/types.ts:618-630`) | — | Five call sites, all via the private `_emitSessionCompactFailed` helper at `agent-session.ts:604-608`: `:2083`, `:2184`, `:2292`, `:2347`, `:2418`. **Gated on `hasHandlers("session_compact_failed")` (`:605`)** — not constructed when nothing subscribes. |
...
| `ui_prompt_start` | **New in 0.85.x.** Pi has **begun blocking** on a user-facing extension UI prompt. Payload: `reason: "ui_prompt"`, `kind: UIPromptKind` (`"select" \| "confirm" \| "input" \| "editor" \| "custom"`, `extensions/types.ts:745`), and optional `title`. **This is the observability hook for the classic "my extension hangs in RPC mode" failure** — see `pi-rpc`'s `reference/extension-ui-bridge.md`: an awaited `ctx.ui` call emits `extension_ui_request` and waits for a matching response, and previously a host had to *infer* that wait. Now it is announced. | `UIPromptStartEvent` (`extensions/types.ts:748-753`) | — (see ⚠ below) | `runner.ts:457`, inside `withUIPrompt` (`runner.ts:453-479`), dispatched via `emitUIPromptEvent` (`runner.ts:482-486`) |
| `ui_prompt_end` | **New in 0.85.x.** Pi is **no longer blocking** on that prompt. Same payload shape as `ui_prompt_start`, replayed from the recorded prompt so `kind`/`title` match the `start` even if state changed meanwhile (`runner.ts:465-471`). | `UIPromptEndEvent` (`extensions/types.ts:756-761`) | — (see ⚠ below) | `runner.ts:465-471` via the `finish()` closure in `withUIPrompt` |

> ⚠ **Three properties of the `ui_prompt_*` pair that determine what you can legitimately build on them.** All three follow from `withUIPrompt` (`runner.ts:453-479`) and `emitUIPromptEvent` (`:482-486`):
>
> 1. **Outermost-only, depth-tracked.** `uiPromptDepth` counts nesting; only the transition 0→1 emits `start` and only 1→0 emits `end` (`:454`, `:461-462`). **Nested `ctx.ui` calls produce exactly ONE pair, not one per call.** Do not treat these as a per-call trace.
> 2. **Balanced even on throw.** `finish()` runs from both `run().finally(finish)` and the `catch` (`:475-479`), so a prompt that rejects still emits `end`. A dangling `start` means the process died, not that the prompt is still open.
> 3. **Fire-and-forget, and NOT awaited.** `emitUIPromptEvent` wraps the emit in `queueMicrotask(() => void this.emit(event))` (`:483-485`). The handler's return value is **discarded**, and the emit is deferred by a microtask relative to the prompt actually opening. So: you **cannot cancel, block, or answer a UI prompt from these handlers** — they are notification only. Anything that needs to *intervene* belongs in the RPC `extension_ui_request` / `extension_ui_response` exchange instead.
