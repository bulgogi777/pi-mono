...
How `pi install` works, what the `packages` array in `settings.json` does, and how npm packages contribute extensions/skills/prompts/themes via a `pi` field in their `package.json`. All cites against pi-mono at the current pin (`v0.84.1`, `53fa77cc`). The CLI handler is `package-manager-cli.ts` (`packages/coding-agent/src/package-manager-cli.ts`); the resolver is `DefaultPackageManager` at `packages/coding-agent/src/core/package-manager.ts:740-…`.
...
|---|---|---|---|
| `pi install <source> [-l]` | `:45` | Resolves the source, installs the package, and persists it to settings via `installAndPersist`. | `package-manager-cli.ts:433-436`; `package-manager.ts:950-…` |
| `pi remove <source> [-l]` (alias `pi uninstall`) | `:48` | `removeAndPersist` — deletes the install and removes from settings. Returns false (`exitCode = 1`) if not found. | `package-manager-cli.ts:438-447` |
...
| `pi list` | `:51` | Lists configured packages by scope (user / project), with `(filtered)` annotation when the entry is in object form. | `package-manager-cli.ts:448-471` |
...
Three source kinds parsed by the package manager (see `package-manager.ts:115-129`):
...
`pi install` examples shown in `--help` (`:61-67`):
...
The `Settings.packages` field (`settings-manager.ts:101`) is `PackageSource[]?` (`:71-79`):
...
Within a single resolution pass, the package manager collects packages from **both** scopes and merges them; project-scope packages win on collision (`package-manager.ts:839-846` and `:1606-1654`).

## The `pi` manifest in `package.json`

When `pi` resolves a package, it reads the package's `package.json` and looks for a `"pi"` field. Shape: `PiManifest` at `extensions/loader.ts:498-503`:
...
`DefaultPackageManager.resolve(onMissing?)` at `package-manager.ts:740-…` (the entry point used by the resource loader at startup):

1. Read both global and project `settings.json` (`package-manager.ts:2141-2147`).
2. For each `PackageSource` in either array, parse and resolve to an installed local path (`getInstalledPath`, `:85-90`). Missing sources can trigger `onMissing` to install on demand or skip.
3. For each installed package, read its `package.json` `pi` field via `readPiManifest` (`extensions/loader.ts:504-514`).
4. Per resource type (`"extensions" | "skills" | "prompts" | "themes"`, `:190-192`), collect the file paths the manifest declares.
...
7. Return a `ResolvedPaths` (`package-manager.ts:70-75`) keyed by resource type, each entry a `ResolvedResource` (`:54-58`) carrying `path`, `metadata`, and `enabled`.

The result feeds `DefaultResourceLoader` (`resource-loader.ts:451-455` for extensions), which then passes the merged file list to `loadExtensions`, `loadSkills`, etc.

## Install internals

`DefaultPackageManager.install(source, options?)` at `package-manager.ts:927-…`:
...
- Surfaces `ProgressEvent` (`:69-74`) via the registered `ProgressCallback` so the CLI can stream `Installing <source>...` lines.

`installAndPersist` (`:950-…`) wraps `install` and then calls `addSourceToSettings` to write the new entry into `settings.json` under a `proper-lockfile` lock.
