...
Top-level dispatcher: `handlePackageCommand` at `package-manager-cli.ts:551-…`. Recognized commands (`:27`): `"install" | "remove" | "update" | "list"`. Plus `"uninstall"` as an alias for `"remove"` (`:310-312`).

| Command | Usage | What it does | Code |
|---|---|---|---|
| `pi install <source> [-l]` | `:231` | Resolves the source, installs the package, and persists it to settings via `installAndPersist`. | `package-manager-cli.ts:433-436`; `package-manager.ts:977-…` |
| `pi remove <source> [-l]` (alias `pi uninstall`) | `:58` | `removeAndPersist` — deletes the install and removes from settings. Returns false (`exitCode = 1`) if not found. | `package-manager-cli.ts:624-633` |
| `pi update [source\|self\|pi] [--self] [--extensions] [--extension <s>] [--force]` | `:235` | Updates pi itself (`--self` / `pi`), one package, all packages (`--extensions`), or both (default). | `package-manager-cli.ts:452-526` |
| `pi list` | `:237` | Lists configured packages by scope (user / project), with `(filtered)` annotation when the entry is in object form. | `package-manager-cli.ts:634-657` |
...
Three source kinds parsed by the package manager (see `package-manager.ts:125-139`):
...
`pi install` examples shown in `--help` (`:71-77`):
...
Within a single resolution pass, the package manager collects packages from **both** scopes and merges them; project-scope packages win on collision (`package-manager.ts:866-873` and `:1633-1681`).
...
`DefaultPackageManager.resolve(onMissing?)` at `package-manager.ts:767-…` (the entry point used by the resource loader at startup):
...
The result feeds `DefaultResourceLoader` (`resource-loader.ts:452-456` for extensions), which then passes the merged file list to `loadExtensions`, `loadSkills`, etc.

## Install internals

`DefaultPackageManager.install(source, options?)` at `package-manager.ts:954-…`:
...
- Surfaces `ProgressEvent` (`:79-84`) via the registered `ProgressCallback` so the CLI can stream `Installing <source>...` lines.

`installAndPersist` (`:977-…`) wraps `install` and then calls `addSourceToSettings` to write the new entry into `settings.json` under a `proper-lockfile` lock.
