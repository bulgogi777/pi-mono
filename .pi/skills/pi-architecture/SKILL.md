...
- `reference/settings-json-schema.md` — full `Settings` schema for `~/.pi/agent/settings.json` and `<cwd>/.pi/settings.json` with field-by-field cites against `settings-manager.ts:80-122`, defaults, and the project-vs-global `deepMergeSettings` rules (function at `:126`).
...
- "What does `--no-skills` actually disable?" → `packages/coding-agent/src/core/resource-loader.ts:419` (`const skillPaths = this.noSkills ? ... : ...`). Disables auto-discovered defaults but still honours CLI-supplied `--skill <path>` entries. Same pattern for `noPrompts` and `noContextFiles` (the latter gates `loadProjectContextFiles` at `:464`). CLI flag wiring at `args.ts:163-170` (`--no-skills`/`-ns`, `--no-context-files`/`-nc`).
...
- "Where do I add a `--approve` override or set the default for non-interactive runs?" → `--approve` / `-a` (and `--no-approve` / `-na`) at `args.ts:180-183`; `defaultProjectTrust` in `~/.pi/agent/settings.json` (`"ask"` (default) / `"always"` / `"never"`).
