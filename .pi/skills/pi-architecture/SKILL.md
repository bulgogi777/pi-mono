...
- `reference/settings-json-schema.md` — full `Settings` schema for `~/.pi/agent/settings.json` and `<cwd>/.pi/settings.json` with field-by-field cites against `settings-manager.ts:91-135`, defaults, and the project-vs-global `deepMergeSettings` rules (function at `:139`).
...
- "Where is settings.json / how do I read it?" → `getSettingsPath()` at `packages/coding-agent/src/config.ts:552-554`; loader is `packages/coding-agent/src/core/settings-manager.ts`.
...
- "What does `--no-skills` actually disable?" → `packages/coding-agent/src/core/resource-loader.ts:468` (`const skillPaths = this.noSkills ? ... : ...`). Disables auto-discovered defaults but still honours CLI-supplied `--skill <path>` entries. Same pattern for `noPrompts` and `noContextFiles` (the latter gates `loadProjectContextFiles` at `:513`). CLI flag wiring at `args.ts:188-195` (`--no-skills`/`-ns`, `--no-context-files`/`-nc`).
- "What is `CONFIG_DIR_NAME`?" → `packages/coding-agent/src/config.ts:504` — defaults to `.pi`, overridable via `pkg.piConfig.configDir` for rebranded distributions. **Stop hardcoding `.pi` in extensions** — import `CONFIG_DIR_NAME` from `@earendil-works/pi-coding-agent` instead (exported since 0.79.7; see `coding-agent/src/index.ts:7`). This also makes downstream code safe across forks that ship under a different config dir.
- "Where is `~/.pi/agent/`? Can I move it?" → `getAgentDir()` at `config.ts:528-534`. Override with the `PI_CODING_AGENT_DIR` env var (`ENV_AGENT_DIR`, `config.ts:508`).
...
- "Where do I add a `--approve` override or set the default for non-interactive runs?" → `--approve` / `-a` (and `--no-approve` / `-na`) at `args.ts:219-222`; `defaultProjectTrust` in `~/.pi/agent/settings.json` (`"ask"` (default) / `"always"` / `"never"`).
