...
- `reference/settings-json-schema.md` — full `Settings` schema for `~/.pi/agent/settings.json` and `<cwd>/.pi/settings.json` with field-by-field cites against `settings-manager.ts:76-114`, defaults, and the project-vs-global `deepMergeSettings` rules.
