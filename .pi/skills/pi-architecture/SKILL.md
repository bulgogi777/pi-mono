...
- "What does `--no-skills` actually disable?" → `packages/coding-agent/src/core/resource-loader.ts:416` (`const skillPaths = this.noSkills ? ... : ...`). Disables auto-discovered defaults but still honours CLI-supplied `--skill <path>` entries. Same pattern for `noPrompts` and `noContextFiles` (the latter gates `loadProjectContextFiles` at `:464`). CLI flag wiring at `args.ts:163-170` (`--no-skills`/`-ns`, `--no-context-files`/`-nc`).
