...
- "When does compaction fire?" → `shouldCompact()` at `core/compaction/compaction.ts:235-238`: triggers when `contextTokens > contextWindow - reserveTokens`. Defaults `enabled: true, reserveTokens: 16384, keepRecentTokens: 20000` (`DEFAULT_COMPACTION_SETTINGS` at `core/compaction/compaction.ts:132-136`). Since 0.79.8 the `CompactionResult` carries `estimatedTokensAfter` (`compaction.ts:133`) — a heuristic post-compaction estimate, **not provider-exact**.
