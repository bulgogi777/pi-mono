...
- "Where does pi store sessions?" → `~/.pi/agent/sessions/--<encoded-cwd>--/<ts>_<uuid>.jsonl`. Encoding logic at `session-manager.ts:428-435` (`getDefaultSessionDir`); root path via `getSessionsDir()` at `config.ts:395`; override with `PI_CODING_AGENT_SESSION_DIR` (env var `ENV_SESSION_DIR` at `config.ts:330`).
