...
- **Current pin:** `d981de12` (2026-09-06, tag `v0.85.1`; covers releases 0.84.2 → 0.85.1). Matches the installed npm runtime `@earendil-works/pi-coding-agent@0.85.1`. **The consumer-facing changes of this range are ADDITIVE on the wire:** new `clear_queue` command (0.84.4) returning the drained `{steering, followUp}` text; `message_update` gains a top-level `usage`; `toolcall_start` gains `id` + `toolName` (`modes/json-event.ts:24-30`). **`abort` did NOT change** — `modes/rpc/rpc-mode.ts:428-431` is byte-identical to `v0.84.1`; 0.85.0 only widened what *idle* means, folding compaction/branch-summary into `isIdle` (`core/agent-session.ts:925-927`, was `:883-885`; `abort()` at `:1619-1625`; commit `bea67d90d`, upstream #8920), so an abort during compaction settles slower. *(An earlier revision of this line called `abort` a behavioral change that "previously returned immediately" — wrong, corrected 2026-09-06; see the version-log entry.)* Also in territory: the skills gate in the system prompt widened from `read`-only to **`read` OR `bash`** (`core/system-prompt.ts:46`, upstream #8552), and `SessionManager.inMemory()` gained an `entries?: FileEntry[]` rehydration arg (`core/session-manager.ts:1600`, #8980). Anthropic OAuth billing survived a +268-line rewrite of `packages/ai/src/api/anthropic-messages.ts` intact (`sk-ant-oat` `:898`, beta headers `:1003`) — **verified live, not by grep**: see the version-log entry for the six-gate result. Trust gating unchanged (`core/resource-loader.ts` diff is BOM-stripping only).
  - **Prior pin:** `53fa77cc` (2026-08-12, tag `v0.84.1`; covers releases 0.83.0 → 0.84.1). Matches the installed npm runtime `@earendil-works/pi-coding-agent@0.84.1`. **The consumer-facing change of this range is the wire, not the models:** `message_update` is now **delta-only** — the cumulative `message` field and `assistantMessageEvent.partial` are stripped by `packages/coding-agent/src/modes/json-event.ts` (new), applied at `packages/coding-agent/src/modes/rpc/rpc-mode.ts:356` (upstream #7290). All four model ids we use survived (`claude-opus-5`, `claude-sonnet-5`, `claude-haiku-4-5`, `claude-opus-4-8`), so **no consumer model flip was required** — see gate 6, which exists because of this range. Anthropic OAuth billing (`packages/ai/src/api/anthropic-messages.ts:844`, `:902`, `:980`), trust gating (`packages/coding-agent/src/core/resource-loader.ts:1024`, `:1038`), and the on-disk session JSONL (`session-manager.ts` is +3/−1 and does **not** adopt the new v4 `SessionRepo`) are all unchanged.
  - **Prior pin:** `b4f29368` (2026-07-25, tag `v0.82.1`; covers releases 0.80.10 → 0.82.1). **Model catalog moved out of the git tree** — `packages/ai/src/providers/*.models.ts` are now thin wrappers importing `./data/<provider>.json`, and `packages/ai/src/providers/data/` is **gitignored** (generated at build from models.dev, shipped only in the npm tarball). Consequence for this expert: *"which models does release X know?" cannot be answered from the pinned tree* — unpack the published tarball (`npm pack @earendil-works/pi-ai@<v>` → `package/dist/providers/data/anthropic.json`). Anthropic OAuth/billing path did NOT move again (`packages/ai/src/api/anthropic-messages.ts`, +37/-8; `sk-ant-oat` detect at :839, `claude-code-20250219,oauth-2025-04-20` headers at :894). `resource-loader.ts` +4 (directory exclusion, #7106) — trust gating intact. Built-in `defaultModelPerProvider.anthropic` is STILL `claude-opus-4-8` (`packages/coding-agent/src/core/model-resolver.ts:17`) — consumer surfaces set `claude-opus-5` explicitly; the built-in default was not changed upstream.
...
### A stale pin is a WRONG-ANSWER bug, not a tidiness bug — and it is silent

**Never "document the checkout as a pinned historical reference" instead of refreshing it.** That option gets proposed every time someone notices the drift, and it is a false choice: for a repo-substrate expert the pin *is* the truth by construction, so freezing it does not preserve accuracy — it relocates the expert's authority to a tree that no longer describes the running system, permanently. The answer is always refresh-and-re-anchor. Recorded here because the alternative was formally proposed in workitem `3d7d1224` (2026-09-06) and declined.

**The failure mode is an absence that reads as an answer.** Measured on that item: `clear_queue` (shipped 0.84.4) had **zero** occurrences anywhere in the checkout while the installed runtime was 0.85.1 and implemented it. An agent grepping the tree to learn pi's RPC surface concludes *the command does not exist* — a confident negative, with nothing anywhere signalling staleness. That is strictly worse than a missing file, which at least fails loudly.

**Two cheap tripwires, either of which catches it without a full eval:**

```bash
# 1. Does the tree match the runtime? Disagreement = stale pin, full stop.
git -C ~/apex/x/code/pi-mono describe --tags        # e.g. v0.84.1-29-g01466e25e
node -p "require('$HOME/.local/lib/node_modules/@earendil-works/pi-coding-agent/package.json').version"

# 2. The consumer's devDependency is an independent witness and often moves FIRST
#    (apex-app bumps it when it adopts a new command).
grep pi-coding-agent ~/apex/x/code/apex-app/package.json
```

Tripwire 2 is the interesting one: on `3d7d1224` the consumer had **already** shipped `clear_queue` support and pinned `0.85.1` while this expert still cited `v0.84.1`. **The consumer moving ahead of the pin is the earliest available signal that a self-update is overdue** — check it whenever an RPC question arrives, not only during version work.
...
6. **Wire shape — the CONSUMER-FACING EVENT CONTRACT, not just the model id** *(added 2026-08-12 on the 0.82.1 → 0.84.1 eval)*. Gates 1-5 all ask "is pi running, billing right, and on the right model?" **None of them reads what pi puts on the wire**, so a release that changes the *shape* of an event passes every one of them while a consumer silently renders the wrong thing. This is not hypothetical: **v0.84.0 removed the cumulative `message` field and `assistantMessageEvent.partial` from `message_update`** (upstream #7290; stripper at `modes/json-event.ts`, applied `modes/rpc/rpc-mode.ts:356`) — a change no model-id-shaped gate could see. Run **before** the pin bump:
   - **Diff the contract, not the code.** `git diff <old-pin>..v$RUNTIME -- packages/coding-agent/docs/rpc.md packages/coding-agent/docs/json-mode.md packages/coding-agent/src/modes/rpc/rpc-types.ts packages/coding-agent/src/modes/json-event.ts`. The docs are maintained per-release and state removals in prose; `rpc-types.ts` alone is **not** sufficient (it had an *empty* diff across 0.82.1→0.84.1 while the event payload changed underneath it).
   - **⚠ A docs delta is a POINTER, not evidence — confirm every behavioural claim against the implementation for that release.** The bullet above says the docs state things the types miss; the inverse is equally true and bit hard on 2026-09-06. `docs/rpc.md` reworded `abort` from "Abort the current agent operation." to "…and wait for the session to become idle before responding," and that was recorded as a behaviour change. It was not: `modes/rpc/rpc-mode.ts:428-431` is **byte-identical** at `v0.84.1` and `v0.85.1`, and the `await` predates both (`3559a43ba`). The actual 0.85.0 change was narrower and in a different file (`bea67d90d`: compaction folded into `isIdle`, `core/agent-session.ts:925-927`). **The wrong claim reached a delegator holding a merge gate before it was caught.** So: when a doc rewords a behaviour, `git diff <old-tag>..<new-tag>` the file that implements it *before* writing it down — a one-command check that would have prevented this.
   - **Grep every consumer for each removed/renamed field**, not just the obvious one. The consumer set is: `apex-app/src/lib/pi-rpc/`, `apex-app/src/lib/chat/session-reducer.ts`, `apex-app/server/`, `~/.claude/scripts/synapse/`, `~/.claude/scripts/panel-consult.ts`, `~/.claude/scripts/voice/`, `~/.claude/scripts/lib/extractor-agent.ts`, `efforts/pi-code/scripts/`.
   - **Check the FIXTURES too, and treat a green suite as unproven.** Hand-rolled event types and test fixtures keep compiling and passing against a shape the wire no longer emits — the tests go on asserting the *old* contract. On this eval that was the single highest-value item found (apex-app `pi-event-router.test.ts`, `rpc-chat-utils.ts`, `mock-pi-rpc.ts` all built `message_update` fixtures carrying the removed fields).
   - **Bump any consumer's `@earendil-works/pi-coding-agent` devDependency in the SAME change — and preferably BEFORE the `npm i -g`.** A consumer whose typecheck gate is pinned to the old version stays green while the runtime drifts: the gate cannot see the version it does not have. Bumping first turns a live-upgrade breakage into a compile error delivered while nothing has changed yet.
...
- Claims about behavior cite `file:line` in the pinned tree — always the **Current pin** declared above (today: `d981de12`, `v0.85.1`), never a prior pin. *This line has drifted before; when bumping the pin, update it here too or the confidence rule silently authorizes stale cites.*
